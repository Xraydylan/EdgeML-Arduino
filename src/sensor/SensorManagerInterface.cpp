...
void SensorManagerInterface::init() {
    ...
    setup_sensors(); // Creates EdgeSensor Objects

    setup_scheme_buffer(); // BLE information schemes
}

EdgeSensor ** SensorManagerInterface::get_sensors() {
    return _sensors;
}
...
void SensorManagerInterface::setup_ID_arrays() {
    ...
    _config_id_index = new const EdgeSensorConfig * [_sensor_count];

    int * counts = new int[_module_count] {0};
    const EdgeSensorConfig * s_con;
    // i -> Index
    for (int i=0; i<_sensor_count; i++) {
        s_con = &_configs[i];
...
void SensorManagerInterface::setup_sensors() {
    _sensors = new EdgeSensor * [_sensor_count];
    EdgeSensor * sensor;
    for (int i=0; i<_sensor_count; i++) {
        ...
        sensor = new EdgeSensor {i, false, false, 0, 0, data_size};
        _sensors[i] = sensor;
    }
}

int SensorManagerInterface::calculate_size(int ID) {
    const EdgeSensorConfig * config = _config_id_index[ID];
...
void SensorManagerInterface::set_sensor_configs(const EdgeSensorConfig * configurations) {
...
EdgeSensorConfig * SensorManagerInterface::get_config(int ID) {
    return (EdgeSensorConfig *) _config_id_index[ID];
}
